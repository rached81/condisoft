    ...
    get_ajax_data("/parametrage/asyn_get_nbalert_stk", param, function (data) {
        if (data.etat == "0") {
            ...
            $("#artrupture").click(function () {
                ...
                param.mode = "R"
                get_ajax_data("/parametrage/asyn_get_alert_stk", param, function (data) {
                    $.buildTable("#alertdialog", "Liste des alertes du stock", data.data, [],null,true);
                    $("#alertdialog table tr.lineitem").each(function () {
                        obj = JSON.parse($(this).attr("obj"));
                        var html = '<td>'
                        html += '<input class="demproQte"  type="text"  placeholder="Stk min : ' + obj.b_artStockminimale + '">'
    ...
    $(elm).each(function () {
        ...
        if (typeof prm[0] != 'undefined' && typeof prm[1] != 'undefined' && typeof prm[2] != 'undefined') {
            ...
            get_ajax_data("/main/asyn_getautoselect", param, function (data) {
                ...
                $.each(data.data, function (index, value) {
                    html +='<option value="' + value[ids] + '">' + value[libs] + '</option>';
                })

                $(elmselect).append(html);
 ...
 * 
 * @param {*} number 
 * @param {*} devise ISO 4217 
 * @returns 
 */
function sgsNumber(number, devise="") {
    
    devise = devise.toUpperCase();

    switch(devise) {
        case 'EURO':
           res =   numeral(number).format('0.00');
          break;
        case 'TND':
            res =   numeral(number).format('0.000');
          break;
          case 'USD':
             res =   numeral(number).format('0.00');
          break;
        default:
            res =  roundDecimal(number, 3);
      }
      return res;
...
function roundDecimal(nombre, precision){
    ...
    number = nombre;
    length = null;
    comma = number.toString().split(".").length

    if(comma == 2){
     
        var tmp = Math.pow(10, precision);
        return (Math.round( nombre*tmp )/tmp).toFixed(precision);
